...
def _image_post_report(signals: Dict[str, Any]) -> Dict[str, Any]:
    ...
    report = {
        ...
        "production_method_inference": [
            {"method": "Canva/Figma 这类静态排版工具（推断）", "confidence": 0.52, "evidence": evidence_pool[:2]},
            {"method": "真实截图 + 文字卡混排（推断）", "confidence": 0.28, "evidence": evidence_pool[2:4]},
            {"method": "AI 概念示意图 + 后期手动排字（推断）", "confidence": 0.2, "evidence": evidence_pool[4:6]},
        ],
        "virality_drivers": virality_drivers[:3],
        "adaptation_ideas": [
            {
                "idea": "保留现有页型节奏，但把前 3 页压到“用户痛点 + 一句话差异 + 核心解释图”。",
                "rationale": "这能最大限度复用结构优势，而不复写原内容。",
            },
            {
                "idea": "中段至少放 2 页具体场景或步骤页，不要整篇停留在观点表达。",
                "rationale": "对小红书来说，收藏来自可执行信息，不来自抽象判断。",
            },
            {
                "idea": "尾页固定保留 CTA：收藏、评论想看下一篇、或具体追问入口。",
                "rationale": "这类尾页是系列化图文的关键转化位。",
            },
        ],
